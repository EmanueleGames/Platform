...
clGameOverState::clGameOverState()
{
    ...
    if (!mGameOverBackground.createFromFile(gRenderer, "data/gameover_background.png"))
    {
        cout << "Background gameover image loading failed!" << endl;
    }

    //Game Over messagges
    stringstream stringFinalScore;
    stringFinalScore.str("");
    stringFinalScore << "Final Score:  " << gScore;
    ...
    while (SDL_PollEvent(&gEvent))
    {
        ...
        else if ((gEvent.type == SDL_KEYDOWN))
        {
            if (gEvent.key.keysym.sym == SDLK_RETURN)
            {
                ...
                set_next_state(eGameStates::STATE_TITLE);
            }
        }
    }
}
...
void clGameOverState::render()
{
    ...
    mTextPlayAgain.render(gRenderer, (WINDOW_WIDTH - mTextPlayAgain.getWidth()) / 2, 620);
}